...
static void experiment() {
  ...
  auto state = emt6ro::loadFromFile("../data/tumor-lib/tumor-1.txt", params);
...
//  protocol_data_h[72 * HOUR_STEPS / PROTOCOL_RES] = 2;
  ...
  Timelapse timelapse(10, 960, 100);
  for (int t = 0; t < 10 * 24 * 4; ++t) {
    simulation.run(HOUR_STEPS / 4);
    for (uint32_t tumor = 0; tumor < 10; ++tumor) {
      for (uint32_t test = 0; test < 100; ++test) {
        ...
        timelapse.val(tumor, t, test, Feature::CHO) = view(27, 27).substrates.cho;
        timelapse.val(tumor, t, test, Feature::OX) = view(27, 27).substrates.ox;
        timelapse.val(tumor, t, test, Feature::GI) = view(27, 27).substrates.gi;
      }
    }
    std::cout << std::endl;
    std::cerr << (float)t * 0.25 << std::endl;
  }
